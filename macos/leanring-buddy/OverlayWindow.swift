...
struct BlueCursorView: View {
    ...
    private let navigationBubbleMaxVisibleCharacters = 84
    private let navigationBubbleMaxWidth: CGFloat = 220
    private let navigationBubbleHorizontalPadding: CGFloat = 14
    private let navigationBubbleVerticalPadding: CGFloat = 18
    private let navigationBubbleEdgePadding: CGFloat = 20
        ...
        ZStack {
                ...
                Text(welcomeText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(currentOverlayTint)
                            .shadow(color: currentOverlayTint.opacity(0.5), radius: 6, x: 0, y: 0)
                    )
                    .fixedSize()
                    .overlay(
            ...
            if buddyNavigationMode == .pointingAtTarget && !navigationBubbleText.isEmpty {
                Text(navigationBubbleText)
                    ...
                    .lineLimit(1)
                    .truncationMode(.head)
                    .frame(maxWidth: navigationBubbleMaxWidth, alignment: .leading)
                    .padding(.horizontal, 8)
                    .padding(.vertical, 4)
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(currentOverlayTint)
                            .shadow(
                                color: currentOverlayTint.opacity(0.5 + (1.0 - navigationBubbleScale) * 1.0),
            ...
            Triangle()
                ...
                .shadow(color: currentOverlayTint, radius: 8 + (buddyFlightScale - 1.0) * 20, x: 0, y: 0)
            ...
            BlueCursorSpinnerView(tint: currentOverlayTint)
    ...
    private var currentOverlayTint: Color {
        companionManager.isResearchModeEnabled
            ? DS.Colors.overlayCursorResearchRed
            : DS.Colors.overlayCursorBlue
    }

    /// Keeps the navigation bubble within the current screen instead of letting
    /// long replies spill off the right or bottom edge.
    private var navigationBubblePosition: CGPoint {
        ...
        let preferredX = cursorPosition.x + navigationBubbleHorizontalPadding + halfWidth
        let preferredY = cursorPosition.y + navigationBubbleVerticalPadding

        return CGPoint(
            x: min(
                max(preferredX, navigationBubbleEdgePadding + halfWidth),
                screenFrame.width - navigationBubbleEdgePadding - halfWidth
            ),
            y: min(
                max(preferredY, navigationBubbleEdgePadding + halfHeight),
                screenFrame.height - navigationBubbleEdgePadding - halfHeight
            )
        )
    }
    ...
    private func streamNavigationBubbleCharacter(
        ...
        let nextFullText = navigationBubbleText + String(phrase[charIndex])
        navigationBubbleText = rollingNavigationBubbleText(from: nextFullText)
    ...
    /// Shows the newest chunk of the spoken reply once the bubble reaches its
    /// size budget, creating a ticker-like effect instead of growing forever.
    private func rollingNavigationBubbleText(from fullText: String) -> String {
        let normalized = fullText
            .replacingOccurrences(of: "\n", with: " ")
            .replacingOccurrences(of: "\r", with: " ")

        guard normalized.count > navigationBubbleMaxVisibleCharacters else {
            return normalized
        }

        let suffixStart = normalized.index(
            normalized.endIndex,
            offsetBy: -navigationBubbleMaxVisibleCharacters
        )
        let suffix = String(normalized[suffixStart...])

        if let firstWhitespace = suffix.firstIndex(where: { $0.isWhitespace }) {
            let wordAlignedStart = suffix.index(after: firstWhitespace)
            return String(suffix[wordAlignedStart...])
        }
