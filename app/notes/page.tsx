...
export default async function NotesPage() {
  ...
  if (projects.length === 0) {
    return (
      <section className="grid min-h-[72dvh] place-items-center border border-[var(--border)] bg-white">
  ...
  return (
    <div className="space-y-6 pb-16">
      {projects.map((project, projectIndex) => {
        ...
        return (
          <section key={project.id} className="reveal border border-[var(--border)] bg-white p-6" style={{ "--index": projectIndex } as CSSProperties}>
            <div className="relative overflow-x-auto">
              <div id={canvasId} className="relative min-w-[1160px]" style={{ height: laneHeight }}>
                ...
                <div data-node="project-card" className="absolute left-0 top-0 w-[320px] border border-[var(--border)] bg-[#fbfbfa] p-6">
                  ...
                  <h2 className="mt-2 flex items-center gap-2 text-2xl font-semibold tracking-[-0.03em]">
                    ...
                    <Link href={`/notes/${project.id}`} className="underline-offset-4 hover:underline">{project.title}</Link>
                  </h2>
                ...
                {layout.map(({ session, centerY, blockHeight }) => (
                  <div key={session.id} className="absolute" style={{ left: 380, top: centerY - 7 }}>
                    <div data-node="session-dot" data-session-id={session.id} className="group/session relative h-4 w-4 rounded-full border border-[var(--foreground)]/50 bg-white">
                      <div className="pointer-events-none absolute -left-10 -top-10 hidden rounded-[6px] border border-[var(--border)] bg-white px-2 py-1 text-[11px] text-[var(--muted-foreground)] group-hover/session:block">{new Date(session.sessionDate).toLocaleDateString()}</div>
                    </div>
                    <div className="absolute left-[184px] w-[500px] space-y-2" style={{ top: -(blockHeight / 2) }}>
                      {session.notes.length > 0 ? session.notes.map((note) => (
                        <Link key={note.id} data-node="note-card" data-note-id={note.id} href={`/notes/${project.id}/sessions/${session.id}/notes/${note.id}`} className="block border border-[var(--border)] bg-[#fcfcfa] px-3 py-2 text-sm transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:translate-x-[2px] hover:bg-white">{note.title}</Link>
