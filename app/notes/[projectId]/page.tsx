      ...
      <section className="border border-[var(--border)] bg-white p-6 md:p-8">
        <div id={canvasId} className="relative min-h-[72dvh] w-full overflow-x-auto">
          <div className="relative min-w-[1120px]" style={{ height: mapHeight }}>
            ...
            {rows.map(({ session, topY }) => (
              <div key={session.id} className="absolute" style={{ left: 32, top: topY }}>
                <Link
                  ...
                  className="block w-[240px] border border-[var(--border)] bg-[#fbfbfa] p-3 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:scale-[1.02]"
                >
                  ...
                  {session.notes.length > 0 ? (
                    session.notes.map((note) => (
                      <Link
                        ...
                        className="block min-h-[84px] border border-[var(--border)] bg-white px-3 py-2 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:translate-x-[2px] hover:bg-[#fdfdfc]"
                      >
            ...
            {rows.length === 0 ? (
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 border border-[var(--border)] bg-[#fcfcfa] p-6 text-center">
